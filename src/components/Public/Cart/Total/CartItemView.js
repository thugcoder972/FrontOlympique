...
const CartItemView = observer(({ id, image, title, price, quantity = 0 }) => {
      ...
      <div className="cartItem">
        ...
        <div className="cartItem__info">
          ...
          <div className='cartItem__incrDec'>
            <button onClick={() => cartViewModel.decrementQuantity(id)}>-</button>
            <p>{quantity}</p>
            <button onClick={() => cartViewModel.incrementQuantity(id)}>+</button>
          </div>
          <button
            className='cartItem__removeButton' 
            onClick={() => cartViewModel.removeItem(id)}>
            Remove
          </button>
        </div>
      </div>
